...
const MarqueeStack = () => {
  ...
  return (
    <div className="w-full col-span-2 border border-zinc-400 dark:border-slate-600 rounded-md p-4 flex justify-center items-center hover:border-cyan-400 hover:scale-105 duration-300">
<div className="w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-128px),transparent_100%)]">
      <ul className="flex items-center justify-center md:justify-start [&_li]:mx-8 [&_img]:max-w-none animate-infinite-scroll">
        {techStack.map((tech, index) => (
          <li
            key={index}
            className="flex items-center gap-2 text-zinc-700 dark:text-gray-400 hover:text-cyan-400 transition-colors duration-300"
          >
            {tech.icon}
            <span>{tech.name}</span>
          </li>
